...
@section('title', 'Edit Service') <!-- Sets the title for the edit page -->
...
@section('content')
    <div class="container mt-5">
        ...
        <h3 class="text-center text-orange">Edit Service</h3> <!-- Page title -->
        ...
        <form action="{{ route('services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="price" name="price" value="{{ old('price', $service->price) }}" step="0.01">
                @if ($errors->has('price'))
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="contact" name="contact" value="{{ old('contact', $service->contact) }}">
                @if ($errors->has('contact'))
                    <span class="text-danger">{{ $errors->first('contact') }}</span>
                @endif
            </div>

            <!-- Display the current service image if available -->
            @if($service->service_picture)
                <div class="mb-3">
                    <label for="current_picture" class="form-label">Current Image</label>
                    <div class="text-center">
                        <img src="{{ asset('storage/' . $service->service_picture) }}" alt="Service Image" class="img-fluid" style="max-width: 100%; height: auto; border-radius: 15px;">
                    </div>
                    <div class="form-check mt-2">
                        <input type="checkbox" class="form-check-input" id="delete_picture" name="delete_picture" value="1">
                        <label class="form-check-label" for="delete_picture">Delete the current image</label>
                    </div>
                </div>
            @endif
            ...
            <div class="mb-3">
                <label for="service_picture" class="form-label">Upload New Image (Optional)</label>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            updateSubcategories(); // Initial call to set subcategories on page load
        });

        var map = L.map('map').setView([{{ old('latitude', $service->latitude) ?? 46.3604544 }}, {{ old('longitude', $service->longitude) ?? 25.7992867 }}], 13);

        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '© OpenStreetMap contributors'
        }).addTo(map);

        var marker = L.marker([{{ old('latitude', $service->latitude) ?? 46.3604544 }}, {{ old('longitude', $service->longitude) ?? 25.7992867 }}]).addTo(map);
